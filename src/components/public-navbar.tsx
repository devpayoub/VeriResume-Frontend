    ...
    <div className="fixed top-5 left-0 right-0 z-50 flex justify-center px-4 pointer-events-none">
      <header className="w-full max-w-5xl bg-[#e8e4dc]/40 backdrop-blur-2xl border border-white/20 rounded-full py-2.5 px-5 flex items-center justify-between shadow-[0_4px_24px_rgba(0,0,0,0.05)] pointer-events-auto">
        ...
        <Link href="/" className="flex items-center gap-2 shrink-0">
          ...
          <span className="text-[15px] font-semibold tracking-tight text-[#1a1916] hidden xs:block">
            VeriResume
          </span>
        </Link>
        ...
        <div className="flex items-center gap-3 sm:gap-5">
          <Link 
            href="https://github.com/devpayoub/VeriResume-Frontend" 
            target="_blank" 
            rel="noopener noreferrer"
            className="text-[#6b6560] hover:text-[#1a1916] transition-colors"
          >
            <svg fill="currentColor" viewBox="0 -0.5 25 25" className="w-5 h-5 sm:w-6 sm:h-6" xmlns="http://www.w3.org/2000/svg">
          ...
          {user ? (
            <Button asChild className="rounded-full px-4 sm:px-5 h-8 sm:h-9 bg-[#3a6b60] hover:bg-[#2e554c] text-white font-semibold text-[12px] sm:text-[13px] shadow-none border-0">
              <Link href="/dashboard">Dashboard</Link>
            </Button>
          ) : (
            <Button asChild className="rounded-full px-4 sm:px-5 h-8 sm:h-9 bg-[#c97b4b] hover:bg-[#b56a3a] text-white font-semibold text-[12px] sm:text-[13px] shadow-none border-0">
